...
"""This module contains a script that starts a Flask web application:
    ...
    Use 8-index.html content as source code for the template 100-hbnb.html:
        ...
        Make sure all HTML tags from objects are correctly
        used (example: <BR /> must generate a new line)
        State, City, Amenity and Place objects must be loaded
...
def hbnb():
    ...
    place_list = sorted(storage.all(Place).values(), key=lambda x: x.name)
    st_amen_p = [state_list, amenity_list, place_list]
    return render_template('100-hbnb.html', st_amen_p=st_amen_p)
